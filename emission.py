...
def group_dfs(df_all):
    ...
    df_all["Retentionszeit"] = (df_all["Retention Time"].astype(str).str.replace(',', '.', regex=False).pipe(pd.to_numeric, errors='coerce') / 0.11).round() * 0.11

    df_all["Amount"] = (df_all["Amount "].astype(str).str.replace(',', '.', regex=False).pipe(pd.to_numeric, errors='coerce'))

    df_all["Prob"] = (df_all["Prob "].astype(str).str.replace(',', '.', regex=False).pipe(pd.to_numeric, errors='coerce'))

    df_all["Est. Amt."] = (df_all["Est. Amt."].astype(str).str.replace(',', '.', regex=False).pipe(pd.to_numeric, errors='coerce'))
    ...
    summary[["mean", "td8", "sx", "P"]] = summary[["mean", "td8", "sx", "P"]].round(1)
    summary.rename(columns={'Peak Name': 'Komponente', 'Retentionszeit': 'Retentionszeit', 'mean': 'Toluol Äq.', 'sx': 'Standardabweichung', 'P': 'Probability', 'td8': 'int. Std. Äq.'}, inplace=True)
...
def plot_spectra(df_7, df_28, slider, on_7, on_28):
    x_7 = df_7["Time"]
    y_7 = df_7["Value"]
    x_28 = df_28["Time"] + 0.5
    y_28 = df_28["Value"] - 0.1*10**9
...
plot = plot_spectra(df_7, df_28, slider, on_7, on_28)
...
st.write('Component table (7 days)')
